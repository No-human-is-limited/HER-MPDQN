...
# writer = SummaryWriter("runs/hybrid/HER-MP-DQN-hybrid-Moving-v0-1.2")
...
@click.option('--title', default="PA_DQN_HER", help="Prefix of output files", type=str)
...
@click.option('--alpha', default=0.3, help='forward exp range.', type=float)
def run(seed, episodes, batch_size, gamma, inverting_gradients, initial_memory_threshold, replay_memory_size,
    ...
    path = "results/hybrid/PA_DQN_HER/{}".format(str(seed))
    ...
    for i in range(episodes):
        ...
        if (i+1) % 10 == 0:
            ...
            #         best_succ = succ_rate
            print('{0:5s} R10:{1:.4f} r10:{2:.4f} success rate:{3:.4f}'.format(str(i + 1), np.array(total_re[-10:]).mean(),
                                                                              np.array(returns[-10:]).mean(),
                                                                              (np.array(returns) == 1.).sum() / len(
                                                                                  returns)))
        # writer.add_scalar('r100(acc)', np.array(returns[-100:]).mean(), i + 1)
        # writer.add_scalar('total(acc)', (np.array(returns) == 1.).sum() / len(returns), i + 1)
