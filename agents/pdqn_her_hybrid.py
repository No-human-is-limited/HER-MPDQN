...
from agents.memory.memory import Memory_True
...
class PDQNAgent(Agent):
    ...
    def __init__(self,
                 ...
                 use_pro=False,
                 alpha=0.3):
        super(PDQNAgent, self).__init__(observation_space, action_space, env)
        self.device = torch.device(device)
        self.num_actions = self.action_space.spaces[0].n
        self.action_parameter_size = env.action_space[1].shape[0]
        self.action_parameter_sizes = np.array([1, 1])
        self.action_max = torch.from_numpy(np.ones((self.num_actions,))).float().to(device)
        self.action_min = -self.action_max.detach()
        self.action_range = (self.action_max - self.action_min).detach()
        self.action_parameter_max_numpy = np.array([1, 1])
        self.action_parameter_min_numpy = np.array([0, -1])
        ...
        self.action_parameter_offsets = [1, 2]
        ...
        self.replay_memory_true = Memory_True(replay_memory_size, observation_space.shape,
                                              (1 + self.action_parameter_size,),
                                              env.goal_shape, next_actions=False)
        self.actor = actor_class(self.observation_space.shape[0] + env.goal_shape, self.num_actions,
                                 self.action_parameter_size,
                                 **actor_kwargs).to(device)
        self.actor_target = actor_class(self.observation_space.shape[0] + env.goal_shape, self.num_actions,
                                        self.action_parameter_size,
                                        **actor_kwargs).to(device)
        hard_update_target_network(self.actor, self.actor_target)
        self.actor_target.eval()

        self.actor_param = actor_param_class(self.observation_space.shape[0] + env.goal_shape, self.num_actions,
                                             self.action_parameter_size, **actor_param_kwargs).to(device)
        self.actor_param_target = actor_param_class(self.observation_space.shape[0] + env.goal_shape, self.num_actions,
                                                    self.action_parameter_size, **actor_param_kwargs).to(device)
        ...
        self.actor_optimiser = optim.Adam(self.actor.parameters(),
                                          lr=self.learning_rate_actor,betas=(0.9, 0.999),eps=1e-8)
        self.actor_param_optimiser = optim.Adam(self.actor_param.parameters(),
                                                lr=self.learning_rate_actor_param,betas=(0.9, 0.999),eps=1e-8)
        # new params
        self.writer = writer
        self._forward = 0
    ...
    def add_sample_true(self, state, action, reward, next_state, terminal, goal, achieve_goal, achieve_goal_new):
        assert len(action) == 1 + self.action_parameter_size
        self._step += 1
        self.replay_memory_true.append(state, action, reward, next_state, terminal=terminal, goal=goal,
                                       achieve_goal=achieve_goal, achieve_goal_new=achieve_goal_new)

    def add_sample_forward(self, state, action, reward, next_state, terminal, goal, achieve_goal, achieve_goal_new):
        assert len(action) == 1 + self.action_parameter_size
        self._forward += 1
        self.replay_memory_forward.append(state, action, reward, next_state, goal, achieve_goal, achieve_goal_new,
                                          terminal=terminal)

    def store_transition(self, transition):
        (state, action, reward, next_state, terminal, goal, achieve_goal, achieve_goal_new) = transition
        self.add_sample_true(state, action, reward, next_state, terminal, goal, achieve_goal, achieve_goal_new)

    def save_episode(self, episode_trans, reward_func):
        for transition_idx, transition in enumerate(episode_trans):
            # We cannot sample a goal from the future in the last step of an episode
            state, action, reward, next_state, terminal, goal, achieve_goal, achieve_goal_new = copy.deepcopy(
                transition)
            self.add_sample_true(state, action, reward, next_state, terminal, goal, achieve_goal, achieve_goal_new)
            # if self.use_pro:
            #     if reward == 0:
            #         self.add_sample_forward(state, action, reward, next_state, terminal, goal, achieve_goal,
            #                                 achieve_goal_new)
            if transition_idx == len(episode_trans) - 1:
                break
            ...
            for ag_index in ag_indexes:
                ...
                state, action, reward, next_state, terminal, goal, achieve_goal, achieve_goal_new = copy.deepcopy(
                    transition)
                g = self.get_ag(ag_index=ag_index, episode_transitions=episode_trans)
                # if self.use_pro:
                #     if -0.3 <= g[0] <= 0.3 and -0.3 <= g[1] <= 0.3:
                #         # Update the reward according to the new desired goal
                #         reward = reward_func(achieve_goal_new, g, None)
                #         self.store_transition(transition=(
                #             state, action, reward, next_state, terminal, g, achieve_goal, achieve_goal_new))
                # else:
                reward = reward_func(achieve_goal_new, g, None)
    ...
    def _sample_achieved_goal_index(self, episode_transitions, transition_idx):
        ...
        end_index = len(episode_transitions)
        # if (transition_idx+50) < len(episode_transitions):
        #     end_index = transition_idx + 50
        selected_idx = np.random.choice(np.arange(transition_idx + 1, end_index))
        return selected_idx

    def get_ag(self, ag_index, episode_transitions):
        ag = episode_transitions[ag_index][-2]
        return ag

    def optimize_td_loss(self):
        if self._step < self.batch_size or self._step < self.initial_memory_threshold:
            return
        random_size = self.batch_size
        # if self.use_pro:
        #     forward_size = min(self._forward, int(self.batch_size * self.alpha))
        #     random_size = self.batch_size - forward_size
        #     states_, actions_, rewards_, next_states_, terminals_, goals_, _, _ = self.replay_memory_forward.sample(
        #         forward_size, random_machine=self.np_random)
        states, actions, rewards, next_states, terminals, goals, _, _ = self.replay_memory_true.sample(random_size,
                                                                                                  random_machine=self.np_random)
        # if self.use_pro:
        #     states = np.concatenate([states, states_], axis=0)
        #     actions = np.concatenate([actions, actions_], axis=0)
        #     rewards = np.concatenate([rewards, rewards_], axis=0)
        #     next_states = np.concatenate([next_states, next_states_], axis=0)
        #     terminals = np.concatenate([terminals, terminals_], axis=0)
        #     goals = np.concatenate([goals, goals_], axis=0)
        ...
        next_states = np.concatenate([next_states, goals], axis=1)
        ...
        with torch.no_grad():
            ...
            target = torch.clamp(target, -100, 0)
