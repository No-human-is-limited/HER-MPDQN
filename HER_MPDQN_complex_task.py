...
@click.option('--load-model', default=30000, help='load_model', type=int)
...
def run(seed, episodes, batch_size, gamma, inverting_gradients, initial_memory_threshold, replay_memory_size,
    ...
    for i in range(episodes):
        ...
        for j in range(max_step):
            ...
            g = np.array(g_new, dtype=np.float32, copy=False)
            if done:
                break
