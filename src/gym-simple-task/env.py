...
class BaseEnv(gym.Env):
    ...
    def reset(self) -> list:
        ...
        # Generate object regions from edges
        r = self.np_random.uniform(0,1) * (-limit-limit) + limit
        random_point = random.choice([(random.choice([self.np_random.uniform(-0.7,-0.4),self.np_random.uniform(0.4,0.7)]),r),
                                      (r,random.choice([self.np_random.uniform(-0.7,-0.4),self.np_random.uniform(0.4,0.7)]))])
        low_t = [random_point[0], random_point[1], self.target_radius]
        high_t = [random_point[0], random_point[1], self.target_radius]
        self.target = Target(*self.np_random.uniform(low_t, high_t))
        # Fix the coordinates and orientation of the agent
        low_a = [0, 0, 0]
        high_a = [0, 0, 0]
    ...
    def compute_reward(self, achieved_goal_next, goal, info):
        # Compute distance between goal and the achieved_goal_next.
        reward = -1
