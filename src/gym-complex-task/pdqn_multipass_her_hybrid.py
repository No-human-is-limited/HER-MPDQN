...
class MultiPassQActor(nn.Module):
    ...
    def forward(self, state, action_parameters):
        ...
        for a in range(self.action_size-1):
            # print(action_parameters[:, self.offsets[a]:self.offsets[a+1]])
...
class MultiPassPDQNAgent(PDQNAgent):
    ...
    def __init__(self,
        ...
        self.actor = MultiPassQActor(self.observation_space.shape[0]+self.env.goal_shape, self.num_actions, self.action_parameter_sizes,
                                     **kwargs['actor_kwargs']).to(device)
        self.actor_target = MultiPassQActor(self.observation_space.shape[0]+self.env.goal_shape, self.num_actions, self.action_parameter_sizes,
                                            **kwargs['actor_kwargs']).to(device)
        hard_update_target_network(self.actor, self.actor_target)
        self.actor_target.eval()
        self.actor_optimiser = optim.Adam(self.actor.parameters(), lr=self.learning_rate_actor,betas=(0.9,0.999))
